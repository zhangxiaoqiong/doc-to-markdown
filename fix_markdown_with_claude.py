...
def fix_markdown_content(file_path, max_retries=3):
    ...
    # 构建修复提示词
    system_prompt = """你是一个资深的企业文档校对专家。
用户会提供一份由 OCR 识别生成的 Markdown 文档。其中存在一些典型的“形近字”或扫描错误。
请通读全文并修正不符合常规企业语境的错别字。
例如：
- 职位类错误："查事长" -> "董事长"
- 行政类错误："统合" -> "综合"、"核一" -> "统一"、"管外" -> "管理"
- 逻辑类错误："眼线" -> "单线" 或符合语境的业务词。

要求：
1. 只修正错别字和语病，绝对保持原有的 Markdown 排版结构、表格及数据原封不动。
2. 不要改变原有意思，不要自行缩减内容。
3. 直接输出修复后的完整 Markdown 文本。"""
