...
def excel_to_markdown_by_sheets(file_path: str) -> dict:
        ...
        try:
            for sheet_name in workbook.sheetnames:
                ...
                if rows and len(rows) > 1:  # Only process sheets with data
                    markdown_lines = _process_rows(rows, sheet_name)
                    if markdown_lines:
...
def excel_to_markdown(file_path: str) -> str:
    ...
    try:
        ...
        for sheet_idx, rows in enumerate(all_rows_list):
            if rows and len(rows) > 1:
                markdown_lines = _process_rows(rows, f"Sheet{sheet_idx + 1}")
                if markdown_lines:
                    all_markdown_lines.extend(markdown_lines)
...
def _process_rows(rows, sheet_name):
